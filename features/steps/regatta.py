...
def selenium_see_regattas(context, num_regattas):
    """Check that there are the specified Regattas present"""
    ...
    # Check each Regatta via it's Name
...
def selenium_add_regattas(context):
    ...
    Would typically do via API

    """
    for row in context.table:
        _create_regatta(context, row)

@when(u'I enter "{regatta_name}" as the Regatta Name')
def selenium_enter_regatta_name(context, regatta_name):
    # Enter the Regatta Name
    text_entry = context.browser.find_element_by_id("newRegatta")
    text_entry.send_keys(regatta_name)
